...
- `references/ai-generation.md` the fast lane: generate a base mesh with text/image-to-3D (Tripo, Meshy, Rodin, or self-hosted TRELLIS-2 / Hunyuan3D-2.1), pick best-of-N, then retopologise, bake and optimise it; plus photogrammetry and Gaussian-splatting for real-world capture, and when to generate versus hand-model versus parametric CAD.
- `references/realtime-3d-web.md` reusable Three.js patterns with code: a scene skeleton with shadows and environment lighting, a spherical camera fly, an x-ray reveal, fixed-lane callouts with leader lines, colour-coded flow lines, the hide-the-overlay-while-moving rule, click versus drag, compressed-asset loaders (Draco/KTX2/Meshopt) with progress, and performance budgeting.
...
1. Decide the target first. Anything organic or one-off is fastest generated (text/image-to-3D) then refined. A precise manufacturable part points to Fusion 360 parametric CAD. A reusable asset or a photoreal still points to Blender and a GLB. An interactive web viewer points to Three.js where you control everything. Generate or capture the base, refine in Blender, engineer precision in Fusion, ship to the web with Three.js.
